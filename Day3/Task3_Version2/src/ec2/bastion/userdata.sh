...
S3_BUCKET_NAME=$(aws s3api list-buckets --query "Buckets[?contains(Name, 'static')].Name" --output text --region $REGION_CODE)
sed -i "s|S3_BUCKET_NAME|$S3_BUCKET_NAME|g" /home/ec2-user/eks/manifest/product/configmap.yaml
...
/home/ec2-user/scripts/config/docker-image.sh
/home/ec2-user/scripts/config/db-config.sh
