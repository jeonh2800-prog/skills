...
resource "null_resource" "rds" {
  provisioner "local-exec" {
    command = "aws s3 sync ${path.module}/../../src/rds/ s3://${aws_s3_bucket.this.id}/rds/ --delete"
  }
...
resource "null_resource" "delete_secrets" {
  provisioner "local-exec" {
    command = "aws secretsmanager delete-secret --secret-id apdev-rds-secrets --force-delete-without-recovery --region ap-northeast-2"
  }
