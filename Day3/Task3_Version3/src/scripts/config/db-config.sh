...
MYSQL_PASSWORD=$(aws secretsmanager get-secret-value --secret-id $SECRET_NAME --query "SecretString" --output t`ext `--region $REGION_CODE | jq -r ".MYSQL_PASSWORD")
...
while true; do
    mysql -h $MYSQL_HOST -P $MYSQL_PORT -u $MYSQL_USER -p$MYSQL_PASSWORD -D $MYSQL_DBNAME < /home/ec2-user/rds/init.sql && break
    echo "mysql connect attempt $i failed, retrying in 10s..."
    sleep 10
done